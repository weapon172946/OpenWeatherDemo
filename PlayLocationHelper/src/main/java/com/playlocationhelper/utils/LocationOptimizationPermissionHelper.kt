...
class LocationOptimizationPermissionHelper(
    ...
    private val onFailure: ((locationFailedEnum: LocationFailedEnum) -> Unit)?
) {
            ...
            .checkLocationSettings(
                ...
                if (exception is ResolvableApiException) {
                    ...
                    try {
                        ...
                        onFailure?.invoke(LocationFailedEnum.COULD_NOT_OPTIMIZE_DEVICE_HARDWARE)
                    }
                } else {
                    if (activityWeakReference.get() == null) {
        ...
        if (requestCode == REQUEST_CODE) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                if (locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER)) {
                    ...
                    onFailure?.invoke(LocationFailedEnum.HIGH_PRECISION_LOCATION_NA_TRY_AGAIN_PREFERABLY_WITH_NETWORK_CONNECTIVITY)
                } else {
                    if (activityWeakReference.get() == null) {
                        return
                    }

                    onFailure?.invoke(LocationFailedEnum.LOCATION_OPTIMIZATION_PERMISSION_NOT_GRANTED)
                }
            }
        }
    }
